...
export default function LoyaltyTabScreen() {
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <Text style={styles.info}>
          Odaberite objekt za postavke loyalty programa i nagrade.
        </Text>
      }
      ...
      renderItem={({ item }) => (
        <TouchableOpacity
          style={styles.card}
          onPress={() => router.push(`/owner/loyalty?venueId=${item.id}`)}
        >
          {item.coverPhoto ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.meta}>
              {item.loyalty ? '⭐ Program uključen' : 'Program isključen'}
            </Text>
          </View>
